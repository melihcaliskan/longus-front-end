...
const CategoryList = ({ t, lang, query, categories }) => {
    const { category } = query
    return (
        <Container>
            <Title>{t('categories')}</Title>
            <CategoryItem onClick={() => Router.push(`/all`)} active={!category}>
            ...
            {categories.map((item, index) => {
                ...
                return (
                    <CategoryItem onClick={() => Router.push(`/all?category=${item.slug}`)} active={category === item.slug}>
